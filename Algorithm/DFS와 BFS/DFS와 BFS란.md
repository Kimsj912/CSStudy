...
- DFS 문제
    - 경로의 특징을 저장해야되는 문제
    - 만약에 탐색 트리가 매우 깊은데 찾아야 하는 값도 깊이
- BFS 문제
    - 찾아야하는 값이 루트로부터 멀리 있지 않을 때
        
        (DFS는 깊이를 전부 다 서치하므로 시간이 오래 걸림)
        
    - 최단거리 구해야 하는 문제 (ex) 미로찾기
    - Why?  DFS를 사용하면 처음으로 발견되는 해답이 최단거리가 아닐 수 있지만 BFS로 현재 노드에서 가까운 곳부터 찾기 때문에 경로 탐색 시 먼저 찾아지는 해답이 곧 최단거리가 된다.
